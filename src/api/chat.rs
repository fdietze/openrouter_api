...
impl ChatApi {
                ...
                Err(e) => {
                    ...
                    if retry_count < self.config.retry_config.max_retries {
                        ...
                        let log_message = format!(
                            ...
                            retry_count, self.config.retry_config.max_retries, backoff_ms, e
                        );
                        #[cfg(feature = "tracing")]
                        ...
                        backoff_ms =
                            std::cmp::min(backoff_ms * 2, self.config.retry_config.max_backoff_ms);
                        continue;
                    } else {
    ...
    pub fn chat_completion_stream(&self, request: ChatCompletionRequest) -> ChatStream {
        ...
        let stream = try_stream! {
            ...
            if !response.status().is_success() {
                let status = response.status();
                let body = response.text().await.map_err(|e| Error::StreamingError(format!("Failed to read error body: {}", e)))?;
                Err(Error::ApiError {
                    code: status.as_u16(),
                    message: body,
                    metadata: None,
