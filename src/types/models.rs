...
use serde_json::Value; // Added for per_request_limits
// Removed HashMap import as it's no longer directly used in ModelInfo after changes
...
/// Kept for potential future use, though not directly part of the new ModelInfo.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
...
mod tests {
    ...
    fn test_deserialize_model_info() {
        ...
        assert!(model_info.is_ok(), "Failed to deserialize ModelInfo: {:?}", model_info.err());
        ...
        assert_eq!(model_info.supported_parameters, Some(vec!["max_tokens".to_string(), "temperature".to_string(), "top_p".to_string()]));
        assert_eq!(model_info.canonical_slug.as_deref(), Some("moonshotai/kimi-dev-72b"));
        assert_eq!(model_info.hugging_face_id.as_deref(), Some("moonshotai/Kimi-Dev-72B"));
    }
    ...
    fn test_deserialize_models_response() {
        ...
        assert!(models_response.is_ok(), "Failed to deserialize ModelsResponse: {:?}", models_response.err());
    ...
    fn test_deserialize_all_models_from_api() {
        ...
        let models_response: Result<ModelsResponse, _> = serde_json::from_str(&json_data);
        assert!(models_response.is_ok(), "Failed to deserialize ModelsResponse from API data: {:?}", models_response.err());

        let models_response = models_response.unwrap();
        assert!(!models_response.data.is_empty(), "Model data should not be empty");
        ...
        if let Some(first_model) = models_response.data.first() {
            ...
            assert!(!first_model.id.is_empty(), "First model ID should not be empty");
            assert!(!first_model.name.is_empty(), "First model name should not be empty");
